...
namespace API.Data
{
    ...
    public async Task<PagedList<MessageDto>> GetMessagesForUser(MessageParams messageParams)
     ...
     public async Task<PagedList<MessageDto>> GetMessageThreadPaged(MessageParams messageParams)
        {
            var messages =  _context.Messages
            .Include(u => u.Sender).ThenInclude(p => p.Photos)
            .Include(u => u.Recipient).ThenInclude(p => p.Photos)
            .Where(m =>
            (m.Recipient.UserName == messageParams.Username && m.Sender.UserName == messageParams.RecipientUsername)
            ||
            ...
            //if(await updateUnread(messages, messageParams.Username) == -1){
            //    throw new Exception("could not save to DB all the data");
           // }

            var unreadMessages = messages.Where(m => m.DateRead == null &&
                        m.Recipient.UserName == messageParams.Username).ToList();
            ...
            if (unreadMessages.Any())
            {
             ...
             var messagesDTO = messages.ProjectTo<MessageDto>(_mapper.ConfigurationProvider);
              return await PagedList<MessageDto>.CreateAsync(messagesDTO, messageParams.PageNumber, messageParams.PageSize);
           // return _mapper.Map<IEnumerable<MessageDto>>(messages);
